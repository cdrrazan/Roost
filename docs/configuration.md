...
## Offline / maintenance UI — two layers
...
- **An app container is down or unhealthy** (Caddy is up, its upstream isn't →
  **502/503**). Caddy itself answers with a branded *"temporarily offline"* page.
  This is always on — roost generates `error.html` and mounts it into Caddy; no
  configuration needed.
- **The tunnel is wholly down** (cloudflared stopped, Docker stopped, or the box
  is off → Cloudflare **error 1033**). Now *nothing on your host is reachable* —
  ...
  `roost-maintenance`, one route — `*.<zone>/*` — per routing suffix) that serves
