...
- **`internal/config`** — loads/validates `config.yml`, resolves app paths to absolute dirs, resolves each app to exactly one FQDN (a `worker: true` app instead resolves with an empty FQDN — no route). `FindConfig` resolution order: `--config` flag → `$ROOST_CONFIG` → `./roost.yml` → `~/.roost/config.yml` (first hit wins). The optional top-level `include:` key (glob or list) pulls `apps:` from other files — each included file carries only `apps:` (other keys rejected), its paths resolve against its own dir, and its apps append after the main file's in pattern order. `internal/config/edit.go` mutates the app list while preserving comments (`roost add`/`remove`). The optional top-level `control_host:` names the FQDN that routes to the `roost web` control panel (a Caddy route + tunnel ingress to the host process); unset means the panel stays loopback-only.
