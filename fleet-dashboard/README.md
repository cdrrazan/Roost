...
- **Featured apps** — a pinned strip of up to **two** highlighted apps just below the stat gauges, each with status, reachability, live metrics, and quick start/stop. **Star any app card** to feature it (persisted to `localStorage`); flag `featured: true` in the data for the initial pick, or let it fall back to your first two apps.
